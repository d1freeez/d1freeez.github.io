<template>
  <div class="course-card" @click="onLink(data['id'])">
    <div class="_img">
      ...
      <v-lazy-image v-if="data['respond_file_path']" :src="data['respond_file_path']" alt=""/>
      <v-lazy-image v-else src="@/assets/images/card.png" alt=""/>
    </div>
    <div class="_content">
      <div class="_info">
        <h4>{{ data['name'] || '...' }}</h4>
      ...
      <div class="_type" :class="data['has_discount']?'_discount_disabled':''">
        <p> {{
            data['rounded_price'] === null || data['rounded_price'] === 0 ? 'Бесплатный курс' : parseInt(data['rounded_price']) + ' $'
          }}</p>
      </div>
    </div>
  </div>
